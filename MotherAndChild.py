...
base_url = "https://mosbatesabz.com/product-category/mother-and-child/page/"
...
for page_num in range(1, num_pages + 1):
    ...
    for product_item, price_item in zip(product_items, price_items):
        product_title = product_item.text.strip()  # Get the text inside <h3> tag
        price = price_item.find('bdi').text.strip().replace('تومان', '').replace(',', '').strip()  # Get the price text
        
        # Write data to the Excel worksheet
        worksheet.append([product_title, price])

# Save the Excel file
workbook.save('MotherAndChild.xlsx')

print("Data has been successfully scraped and saved to 'MotherAndChild.xlsx'")
